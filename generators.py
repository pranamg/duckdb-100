...
NUM_EXERCISES = len([k for k in QHA.keys() if k.startswith('q')])

def create_jupyter_notebook(destination_filename='100_DuckDB_exercises.ipynb'):
    ...
    for n in range(1, NUM_EXERCISES + 1):
...
def create_jupyter_notebook_random_question(destination_filename='100_DuckDB_random.ipynb'):
    """ Programmatically create jupyter notebook for random question practice """
...
def create_markdown(destination_filename='100_DuckDB_exercises', with_hints=False, with_solutions=False):
    # Create file name
    if with_hints:
    ...
    for n in range(1, NUM_EXERCISES + 1):
        ...
        if with_solutions:
            mdfile.insert_code(QHA[f'a{n}'], language='sql')
...
def create_rst(destination_filename, with_hints=False, with_answers=False):
    # TODO: use rstdoc python library
    pass

if __name__ == '__main__':
    print(f"Generating files for {NUM_EXERCISES} exercises...")
    create_jupyter_notebook()
    print("Created 100_DuckDB_exercises.ipynb")
    create_jupyter_notebook_random_question()
    print("Created 100_DuckDB_random.ipynb")
    create_markdown()
    print("Created 100_DuckDB_exercises.md")
    create_markdown(with_hints=False, with_solutions=True)
    print("Created 100_DuckDB_exercises_with_solutions.md")
    create_markdown(with_hints=True, with_solutions=False)
    print("Created 100_DuckDB_exercises_with_hints.md")
    create_markdown(with_hints=True, with_solutions=True)
    print("Created 100_DuckDB_exercises_with_hints_with_solutions.md")
    print("Done!")
